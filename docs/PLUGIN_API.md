...
- `load3DEntity(kind, id)` loads a static NPC mesh and its cache name. `load3DEntity(kind, id, type)` is required for object meshes so the correct object type is assembled. Call either method off the Swing and render threads because cache model assembly may be expensive.
...
- `entityHoverText(kind, id, type)` returns colored `Map3DTextSegment` values for the existing top-left hover display. The two-argument overload remains available for plugins that do not need object types. Core continues to own the default NPC hover display.
