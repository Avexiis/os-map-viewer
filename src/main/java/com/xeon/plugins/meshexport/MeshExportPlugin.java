...
public final class MeshExportPlugin implements MapViewerPlugin, Map3DLayer
{
	...
	@Override public String id() { return "mesh-export"; }
	@Override public String displayName() { return "Mesh Export"; }
	@Override public void install(PluginContext context)
	{
		this.context = context;
		panel = new MeshExportPanel(context, this::clearSelection);
		SwingUtilities.invokeLater(context::showRightSidebar);
	}
	@Override public boolean entityPickingEnabled() { return true; }
	@Override public Color objectHoverOutlineColor() { return new Color(0x55D6BE); }
	@Override public boolean tileHoverSelectorVisible(boolean entityHovered) { return !entityHovered; }
	@Override public JComponent rightComponent() { return panel; }
	...
	{
		if (!event.shiftDown() || event.button() != MouseEvent.BUTTON1 || entity.kind() != Map3DEntity.Kind.OBJECT) return false;
		...
		try
		{
			...
			else
			{
				...
				if (mesh == null || mesh.faceCount() == 0) throw new IllegalArgumentException("No object mesh is available");
			...
			refreshSelectionOverlay();
			if (selection.isEmpty()) panel.clear();
			else panel.select(selection.values().stream().map(SelectedObject::entity).toList());
			context.showRightSidebar();
	...
	private void clearSelection()
	{
		...
		if (context != null) context.repaintVisible();
	}
