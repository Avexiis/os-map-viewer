...
final class MeshConnectivity
{
	...
	{
		if (!boundsTouch(first, second)) return false;
		for (int a = 0; a < first.faceCount(); a++)
		{
			...
			for (int b = 0; b < second.faceCount(); b++)
			{
				...
				if (!boundsTouch(ta, tb)) continue;
				if (triangleDistanceSquared(ta, tb) <= TOLERANCE_SQUARED) return true;
			}
		}
		return false;
	}
	...
	static boolean connected(List<Map3DMesh> meshes)
	{
		...
		while (!queue.isEmpty())
		{
			...
			{
				if (reached[candidate] || !touches(meshes.get(current), meshes.get(candidate))) continue;
	...
	private static boolean boundsTouch(Map3DMesh a, Map3DMesh b)
	{
		...
		for (int axis = 0; axis < 3; axis++)
		{
			if (bounds[0][axis + 3] + TOLERANCE < bounds[1][axis]
				|| bounds[1][axis + 3] + TOLERANCE < bounds[0][axis]) return false;
		}
		return true;
	}
	...
	private static double triangleDistanceSquared(Vec3[] a, Vec3[] b)
	{
		for (int edge = 0; edge < 3; edge++)
		{
			if (segmentIntersectsTriangle(a[edge], a[(edge + 1) % 3], b)
				|| segmentIntersectsTriangle(b[edge], b[(edge + 1) % 3], a)) return 0;
		}
		double best = Double.POSITIVE_INFINITY;
		for (Vec3 point : a) best = Math.min(best, pointTriangleDistanceSquared(point, b));
		for (Vec3 point : b) best = Math.min(best, pointTriangleDistanceSquared(point, a));
		for (int ae = 0; ae < 3; ae++) for (int be = 0; be < 3; be++)
	...
	private static double pointTriangleDistanceSquared(Vec3 point, Vec3[] triangle)
	{
		...
		if (vc <= 0 && d1 >= 0 && d3 <= 0) return point.minus(a.plus(ab.times(d1 / (d1 - d3)))).lengthSquared();
		Vec3 cp = point.minus(triangle[2]);
		double d5 = ab.dot(cp), d6 = ac.dot(cp);
		if (d6 >= 0 && d5 <= d6) return cp.lengthSquared();
		double vb = d5 * d2 - d1 * d6;
		if (vb <= 0 && d2 >= 0 && d6 <= 0) return point.minus(a.plus(ac.times(d2 / (d2 - d6)))).lengthSquared();
	...
	private static double segmentDistanceSquared(Vec3 p1, Vec3 q1, Vec3 p2, Vec3 q2)
	{
		...
		if (a <= 1e-20 && e <= 1e-20) return r.lengthSquared();
		if (a <= 1e-20) { s = 0; t = clamp(f / e); }
		else
		{
			double c = d1.dot(r);
			if (e <= 1e-20) { t = 0; s = clamp(-c / a); }
			else
			{
	...
	{
		double get(int axis) { return axis == 0 ? x : axis == 1 ? y : z; }
		Vec3 plus(Vec3 value) { return new Vec3(x + value.x, y + value.y, z + value.z); }
		Vec3 minus(Vec3 value) { return new Vec3(x - value.x, y - value.y, z - value.z); }
		Vec3 times(double value) { return new Vec3(x * value, y * value, z * value); }
		double dot(Vec3 value) { return x * value.x + y * value.y + z * value.z; }
		Vec3 cross(Vec3 value) { return new Vec3(y * value.z - z * value.y, z * value.x - x * value.z, x * value.y - y * value.x); }
